...
public:
	...
	uint16_t getBaseline( void );  // 锟斤拷没锟斤拷锟�
	status setBaseline( uint16_t );  //锟斤拷锟矫伙拷锟斤拷
	status enableInterrupts( void );   //锟斤拷锟斤拷锟叫讹拷
	status disableInterrupts( void );  //锟截憋拷锟秸讹拷
	status setDriveMode( uint8_t mode );//锟斤拷锟矫硷拷驶模式
	status setEnvironmentalData( float relativeHumidity, float temperature ); //set Environ mental Data
	void setRefResistance( float );
	status readNTC( void );
	uint16_t getTVOC( void );
	uint16_t getCO2( void );
	float getResistance( void ); //锟矫碉拷锟街匡拷
	float getTemperature( void ); //锟矫碉拷锟铰讹拷
